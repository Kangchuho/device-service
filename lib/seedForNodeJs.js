...
// martCode -> keyCode
function getEncryptionKey(martCode) {
    let key1 = Math.floor(martCode / 1000);
    let key2 = Math.floor((martCode % 1000) / 100);
    let key3 = Math.floor((martCode % 100) / 10);
    let key4 = martCode % 10;
    let strKey = "";
    ...
    for (let i = 0; i < 4; i++) {
      ...
      key3 = ((key3 * 7) + 7) % 10;
      key4 = ((key4 * 8) + 8) % 10;
      strKey += `${key1}${key2}${key3}${key4}`;
    }
    
    return strKey;
}

// KISA SEED 암호화 <-> 투게더스 시스템에 적용된 복호화 가능한 로직
function getEncValue(martCode, value) {
    const keyText = getEncryptionKey(martCode); //KISA_SEED_ECB.java에서 key값은 16byte 고정
    const keyHex = string_to_utf8_hex_string(keyText);
    const key = CryptoJS.enc.Hex.parse(keyHex);
    const encrypted = CryptoJS.SEED.encrypt(value, key, { mode: CryptoJS.mode.ECB, padding: CryptoJS.pad.ZeroPadding });
    return encrypted.toString();
}
// KISA SEED 복호화
function getDecValue(martCode, encordedValue) {
    const keyText = getEncryptionKey(martCode); 
    const keyHex = string_to_utf8_hex_string(keyText);
    const key = CryptoJS.enc.Hex.parse(keyHex);
    const decrypted = CryptoJS.SEED.decrypt(encordedValue, key, { mode: CryptoJS.mode.ECB, padding: CryptoJS.pad.ZeroPadding });
...
var plainText = "192.168.123.1";
var plainValue = CryptoJS.enc.Utf8.parse(plainText); // 평문

var keyText = "4378026229946000"; //KISA_SEED_ECB.java에서 key값은 16byte 고정
...
console.log("--------------복호화------------");
console.log("공통키 : " + keyText);
var cipherText = "NhxUCHmigZIiCF6OXrDvQQ==";
...
const planV = '192.168.123.1';
var encrypted1 = CryptoJS.SEED.encrypt(planV, key, { mode: CryptoJS.mode.ECB, padding: CryptoJS.pad.ZeroPadding });
var decrypted1 = CryptoJS.SEED.decrypt(encrypted1, key, { mode: CryptoJS.mode.ECB, padding: CryptoJS.pad.ZeroPadding });
const encryptedV = encrypted1.toString();
const decryptedV = CryptoJS.enc.Utf8.stringify(decrypted1).replace(/\u0000/g,'');
